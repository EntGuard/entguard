...
CREATE OR REPLACE FUNCTION notify_device_session_update ()
    RETURNS TRIGGER
    AS $$
DECLARE
BEGIN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_trigger WHERE tgname = 'trigger_device_sessions_update') THEN
        CREATE CONSTRAINT TRIGGER trigger_device_sessions_update
        AFTER DELETE OR UPDATE OF session_id ON devices INITIALLY DEFERRED
        FOR EACH ROW EXECUTE PROCEDURE notify_device_session_update ();
    END IF;
END
$$;
...
DROP TRIGGER IF EXISTS trigger_device_sessions_update ON devices restrict;
DROP FUNCTION IF EXISTS notify_device_session_update restrict;
